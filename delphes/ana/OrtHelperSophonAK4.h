#ifndef OrtHelperSophonAK4_h
#define OrtHelperSophonAK4_h
...
class OrtHelperSophonAK4 {
public:
    OrtHelperSophonAK4(std::string model_path, bool debug = false) {
...
private:
    ...
    void make_input(std::map<std::string, std::vector<float>>& particleVars, std::map<std::string, float>& jetVars) {
        ...
        for (size_t i = 0; i < particleVars["part_px"].size(); i++) {
            ...
            input_feats_["part_pt_log"].push_back(std::log(input_feats_["part_pt"][i]));
            input_feats_["part_e_log"].push_back(std::log(particleVars["part_energy"][i]));
