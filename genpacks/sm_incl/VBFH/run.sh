...
if [[ $MACHINE == "farm" ]]; then
    ...
    MG5_PATH=/scratchfs/cms/tyyang99/env_el9/MG5_aMC_v2_9_18
    LOAD_ENV_PATH=/publicfs/cms/user/tyyang99/HH_4b/load_custom_el9_env.sh
fi
...
###### run in cc7 env ######
cat <<EndOfTestFile > run_subprocess_cc7.sh
#!/bin/bash -xe
env -i ./runcmsgrid.sh $NEVENT $RAND 1
EndOfTestFile

chmod +x run_subprocess_cc7.sh
export SINGULARITY_CACHEDIR="/tmp/$(whoami)/singularity"
singularity run -B /publicfs -B /cvmfs -B /etc/grid-security -B /etc/pki/ca-trust --home $PWD:$PWD /cvmfs/unpacked.cern.ch/registry.hub.docker.com/cmssw/cc7:x86_64 $(echo $(pwd)/run_subprocess_cc7.sh)
###### end of run in cc7 env ######
